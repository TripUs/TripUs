		...
		try {
			...
			for(int i=1; i<=daynum; i++) {
				...
				if(!file.getOriginalFilename().equals("")) {
					...
					file.transferTo(f);
					String fileName="http://203.236.209.213:8080/tripus/resources/upload/tripnote/" + idx + "/" + file.getOriginalFilename();
...
//	}
	...
	public String noteComment(@PathVariable int idx, @RequestParam String comment, Model model, HttpSession session) {
		UserDto userInfo = (UserDto) session.getAttribute("userInfo");
		...
		return "redirect:../noteDetail/"+idx;
	}
