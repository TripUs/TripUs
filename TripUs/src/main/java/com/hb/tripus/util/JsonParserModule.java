			...
			for(int i = 0 ; i < itemArray.size(); i++) {
				...
				for (int j = 0; j < dto.getTags().length; j++) {
					if(data.get(dto.getTags()[j]) != null)
						...
						if(dto.getTags()[j].equals("firstimage")) value[j] = "http://localhost:8060/tripus/resources/imgs/no_img.jpg";
						else if(dto.getTags()[j].equals("firstimage2")) value[j] = "http://localhost:8060/tripus/resources/imgs/no_img.jpg";
						else value[j] = "데이터 없음";
					}
				}
